...
Your Choice:'''
    ...
    if choice == 1:
        ...
        if index == -1:
            ...
            logging.info(repo.employees[index])
    elif choice == 3:
        id = input('ID:')
        index = repo.search_employee(id)
        if index == -1:
            ...
            logging.info(f'Employee {repo.employees[index].name} salary updated successfully')
    elif choice == 4:
        id = input('ID:')
        index = repo.search_employee(id)
        if index == -1:
            ...
            if input('Are you sure to delete(y/n)?').upper() == 'Y':
                ...
                logging.ingo(f'Employee deleted successfully')
    elif choice == 5:
        if len(repo.employees) == 0:
            logging.info('No employee exist.')
        else:
            logging.info('List of Employees:')
            for employee in repo.employees:
                logging.info(employee)
    elif choice == 6:
        logging.info('Thank you for using the application')
    else:
        logging.info('No such option is available. Give proper option.')
