...
Your Choice:'''
    ...
    if choice == 1:
        ...
        old_employee = repo1.search_employee(id)
        if old_employee:
            print('ID already exist.')
        else:
            repo1.add_employee(Employee(id=id, name=name, job_title=job_title, salary=salary))
        ...
        if not old_employee:
            ...
            repo1.update_employee(id, new_salary)
        ...
        if not old_employee:
            ...
            if input('Are you sure to delete(y/n)?').upper() == 'Y':
                repo1.delete_employee(id)
                print(f'Employee deleted successfully')
    elif choice == 5:
        employees = repo1.read_all_employees()
        if len(employees) == 0:
